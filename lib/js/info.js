...
const dataTable = (fips, covidData) => {
    ...
    $('#weekly-data')
        .text('')
        .append('<tr><th>Date</th><th>Cases (+Daily Increase)</th><th>Deaths</th></tr>');
    for (let i = covidData.length-1; i >= 0; i--) {
        ...
        $('#weekly-data')
            .append(
                `<tr>
                    <td>${data.date}</td>
                    <td>${parseInt(data.cases).toLocaleString()} <span class=${data.increase > 0 ? 'increase' : 'decrease'}>(+${data.increase})</span></td>
                    <td>${data.deaths}</td>
                </tr>`)
    }
}

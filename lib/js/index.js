...
const cleanData = (res, currentDate) => {
    ...
    if (withinWeek(currentDate, resDate)) {
        let difference = dayDifference(currentDate, resDate);
...
const mapData = async guidelineData => {
    ...
    const data = await d3.csv(DATA_URL, (res) => cleanData(res, CURRENT_DATE)) 
    ...
    counties.forEach(county => {
        ...
        g 
            ...
            .on('mouseover', function() {
                ...
                let dailyIncrease = selection.increase;
                let weeklyIncrease = selection.weeklyIncrease;
                toolTip
                    .style('visibility', 'visible')
                    .html(`
                        <p class="subheading">${ct}, ${st}</p>
                        <p class="tooltip-cases">Cases: ${parseInt(cases).toLocaleString()}</span></p>
                        <p class="tooltip-cases">Weekly Increase: +${parseInt(weeklyIncrease).toLocaleString()}</span></p>
                        <p class="tooltip-cases">Daily Increase: +${parseInt(dailyIncrease).toLocaleString()}</span></p>
                    `)
            })
